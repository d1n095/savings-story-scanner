...
function minutesInWindow(
  ...
  const segE = (segEnd.getTime() - dayStart.getTime()) / 60000;

  let from = ruleFrom;
  let to = ruleTo;

  // Hantera fönster som passerar midnatt (t.ex. 22:00–06:00) — räkna del 1
  if (to <= from) to = ruleTo + 24 * 60;

  // Justera segE om det går över midnatt
  let totalE = segE;
  if (segE < segS) totalE = segE + 24 * 60;

  const overlap = Math.max(0, Math.min(totalE, to) - Math.max(segS, from));
  return overlap;
}
