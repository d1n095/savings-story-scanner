...
import type { InsightExpenseRow, InsightShiftRow } from "../types";
...
export function InsightsView() {
  ...
  const insights = useMemo<Insight[]>(() => {
    const out: Insight[] = [];
    const income = (shifts.data ?? []).reduce(
      (s: number, r: InsightShiftRow) => s + Number(r.total_amount || 0),
      0,
    );
    const spend = (expenses.data ?? []).reduce(
      (s: number, r: InsightExpenseRow) => s + Number(r.amount),
      0,
    );
    const subs = (expenses.data ?? []).filter((e: InsightExpenseRow) => e.is_recurring);
    const subsTotal = subs.reduce((s: number, r: InsightExpenseRow) => s + Number(r.amount), 0);
