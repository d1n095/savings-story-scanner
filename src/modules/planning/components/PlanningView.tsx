...
import { analyzeVacation } from "../vacation";
import { expandRotation, ROTATION_PRESETS } from "../rotations";
...
export function PlanningView() {
  ...
  const profile = useQuery({
    ...
    queryFn: async () => {
      ...
      const def = wps?.find((w) => w.is_default) ?? wps?.[0];
      const profileValues: PlanningProfile = {
        hourly_rate: Number(def?.hourly_rate ?? p?.hourly_rate ?? 0),
        tax_rate: Number(def?.tax_rate ?? p?.tax_rate ?? 30),
        ob_rules: def?.ob_rules ?? p?.ob_rules ?? null,
        break_rules: normalizeBreakRules(def?.break_rules),
        work_profile_id: def?.id ?? null,
        work_profile_name: def?.name ?? null,
      };
    ...
    <div className="space-y-6">
      ...
      <div className="glass rounded-2xl p-2">
        <div className="flex flex-wrap items-center gap-1">
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            <button onClick={() => shiftAnchor(1)} className="rounded-lg p-2 hover:bg-white/[0.04]">
              <ChevronRight className="h-4 w-4" />
            </button>
            <button
              onClick={() => setAnchor(new Date())}
              className="ml-1 rounded-lg border border-border px-3 py-1.5 text-xs hover:bg-white/[0.04]"
            >
              Idag
            </button>
          </div>
      ...
      {mode === "week" && (
        <WeekFiller
          ...
          onSaved={() => {
            qc.invalidateQueries({ queryKey: ["planning-shifts"] });
      ...
      {mode === "vacation" && (
        <VacationPanel
          ...
          shifts={shifts.data ?? []}
          onSaved={() => {
            qc.invalidateQueries({ queryKey: ["planning-absences"] });
          }}
        />
      )}
...
function WeekFiller({
  ...
  const [rows, setRows] = useState<WeekRow[]>(() => {
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        from: "08:00",
        to: "16:00",
        break_minutes: defaultBreakFor("08:00", "16:00"),
        off: true,
      };
    });
  });
  ...
  const taxFrac =
    Number(profile?.tax_rate ?? 0.3) > 1
      ? Number(profile?.tax_rate) / 100
      : Number(profile?.tax_rate ?? 0.3);
  ...
  const warnings = useMemo(
    () =>
      detectWarnings(
        ...
        existing.filter((s) => !planned.some((p) => p.date === isoDate(new Date(s.starts_at)))),
      ),
    [planned, existing],
  );
    ...
    mutationFn: async () => {
      ...
      if (datesToReplace.length) {
        const existingInRange = existing.filter((s) =>
          datesToReplace.includes(isoDate(new Date(s.starts_at))),
        );
  ...
  return (
    <div className="glass space-y-4 rounded-2xl p-5">
            ...
            >
              <div className="col-span-3 text-sm">
                <div className="font-medium">
                  {WEEKDAY_LABELS[(d.getDay() + 6) % 7]} {d.getDate()}/{d.getMonth() + 1}
                </div>
              ...
              <div className="col-span-4">
                <NumericField
                  ...
                  onChange={(v) =>
                    setRows((prev) =>
                      prev.map((x, j) => (j === i ? { ...x, break_minutes: v ?? 0 } : x)),
                    )
                  }
                  min={0}
                  max={240}
              ...
              <button
                onClick={() =>
                  setRows((prev) => prev.map((x, j) => (j === i ? { ...x, off: !x.off } : x)))
                }
                className={cn(
                  "col-span-1 rounded-lg border px-2 py-1.5 text-[10px] uppercase tracking-wider",
                  r.off
                    ? "border-border text-muted-foreground"
                    : "border-[oklch(0.85_0.12_85/0.4)] text-[oklch(0.85_0.12_85)]",
                )}
              >
                {r.off ? "Ledig" : "På"}
              </button>
      ...
      {warnings.length > 0 && (
        <div className="space-y-1.5">
          {warnings.map((w, i) => (
            <div
              key={i}
              className="flex items-start gap-2 rounded-lg border border-[oklch(0.65_0.12_28/0.4)] bg-[oklch(0.65_0.12_28/0.07)] px-3 py-2 text-xs"
            >
      ...
      <div className="flex justify-end">
        <button
          onClick={() => save.mutate()}
          disabled={save.isPending || !planned.length}
...
function MonthsGrid({
  ...
  return (
    <div
      className={cn(
        "grid gap-4",
        months.length === 1 ? "grid-cols-1" : "sm:grid-cols-2 lg:grid-cols-3",
      )}
    >
      {months.map((m) => (
        <MonthCard
    ...
    <div className="glass rounded-2xl p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="text-xs text-muted-foreground">
          {num(agg.hours, 0)}h · {sek(agg.gross)}
        </div>
      </div>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                d.isRed
                  ? "bg-[oklch(0.65_0.12_28/0.18)] text-[oklch(0.85_0.06_28)]"
                  : "bg-background/40 hover:bg-white/[0.05]",
            ...
            >
              ...
              {hasVac && (
                <Plane className="absolute right-0.5 top-0.5 h-2.5 w-2.5 text-[oklch(0.85_0.12_85)]" />
              )}
            </button>
          );
        })}
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 border-t border-border pt-3 text-[10px]">
        <div>
          ...
          <div className="num">{sek(agg.netEstimate)}</div>
        </div>
        <div>
          <div className="text-muted-foreground">OB</div>
          <div className="num">{sek(agg.ob)}</div>
        </div>
        <div>
          <div className="text-muted-foreground">Pass</div>
          <div className="num">{agg.shiftCount}</div>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className="glass grid grid-cols-3 gap-4 rounded-2xl p-4">
        <div>
          ...
          <div className="num text-2xl">{num(yearH, 0)} h</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
            Året — brutto
          </div>
          <div className="num text-2xl">{sek(yearGross)}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
            Året — netto (est.)
          </div>
          <div className="num text-2xl gold-text">{sek(yearNet)}</div>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        {months.map((m, i) => (
          <button
            ...
            className="glass rounded-2xl p-4 text-left transition hover:ring-1 hover:ring-[oklch(0.85_0.12_85/0.5)]"
          >
        ...
        <>
          ...
          {analysis.lostBase + analysis.lostOB > 0 && (
            <div className="rounded-lg border border-[oklch(0.65_0.12_28/0.4)] bg-[oklch(0.65_0.12_28/0.07)] px-3 py-2 text-xs">
              Den här perioden kostar ungefär{" "}
              <strong>{sek(analysis.lostBase + analysis.lostOB)}</strong> i utebliven inkomst (grund
              + OB).
            </div>
          )}
          {analysis.stretchTips.length > 0 && (
            <div className="space-y-1.5">
              {analysis.stretchTips.map((t, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 rounded-lg border border-[oklch(0.85_0.12_85/0.4)] bg-[oklch(0.85_0.12_85/0.07)] px-3 py-2 text-xs"
                >
...
function PatternApplier({
  ...
  const apply = useMutation({
    mutationFn: async () => {
      ...
      const inserts = expanded.map((e) => {
        const startsAt = new Date(`${e.date}T${e.from}:00`);
        let endsAt = new Date(`${e.date}T${e.to}:00`);
        if (endsAt <= startsAt) endsAt = new Date(endsAt.getTime() + 86400000);
        const c = calculateShift({
          startsAt,
          endsAt,
    ...
    onSuccess: () => {
      toast.success(`${expanded.length} pass tillagda`);
      onSaved();
    },
    onError: (e: Error) => toast.error(e.message || "Kunde inte applicera"),
  });
  ...
  return (
    <div className="glass space-y-4 rounded-2xl p-5">
      ...
      <div className="text-xs text-muted-foreground">
        Genererar <strong className="text-foreground">{expanded.length} pass</strong> för{" "}
      ...
      <div className="flex justify-end">
        <button
          ...
          disabled={apply.isPending || !expanded.length}
