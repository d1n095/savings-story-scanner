...
export function expandWeekly(
  start: Date, end: Date, pattern: WeeklyPattern,
): ExpandedShift[] {
  ...
  for (const d of eachDay(start, end)) {
    ...
    out.push({ date: isoDate(d), from: entry.from, to: entry.to, breakMinutes: entry.break_minutes ?? 0 });
  }
  return out;
}

export function expandRotation(
  start: Date, end: Date, rot: Rotation,
): ExpandedShift[] {
...
export const ROTATION_PRESETS: Rotation[] = [
  ...
  },
  {
    name: "Mån–fre 08–16",
    cycle_weeks: 1,
    weeks_json: [[1,2,3,4,5].map((w) => ({ weekday: w, from: "08:00", to: "16:00", break_minutes: 30 }))],
  },
  {
    name: "Varannan helg 08–20",
    cycle_weeks: 2,
    weeks_json: [
      [{ weekday: 6, from: "08:00", to: "20:00", break_minutes: 30 }, { weekday: 0, from: "08:00", to: "20:00", break_minutes: 30 }],
      [],
    ],
  },
];
