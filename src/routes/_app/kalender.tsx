...
function KalenderPage() {
  ...
  const dayIndex = useMemo(
    () =>
      buildDayIndex(gridStart, gridEnd, {
        shifts: shifts.data as any,
        expenses: expenses.data as any,
        reminders: reminders.data as any,
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Ditt liv på en sida — klicka på en dag för detaljer.
          </p>
        </div>
      ...
      <div className="glass flex flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl px-4 py-2 text-[11px] text-muted-foreground">
        {(
          ["shift", "income", "expense", "reminder", "absence", "holiday", "nameday"] as EventKind[]
        ).map((k) => (
          ...
          {grid.map((d) => {
            ...
            return (
              ...
              >
                ...
                {day?.redDay && (
                  <div className="mt-0.5 line-clamp-1 hidden text-[9px] text-[oklch(0.7_0.12_28)] sm:block">
                    {day.redDay.name}
                  </div>
                )}
                {day?.namedays.length ? (
                  <div className="mt-0.5 line-clamp-1 hidden text-[9px] text-muted-foreground sm:block">
                    {day.namedays.join(", ")}
                  </div>
                ) : null}
                {dots.length > 0 && (
                  <div className="mt-auto flex flex-wrap gap-1 pt-1">
                    {dots.map((e, i) => (
                      <span
                        key={i}
                        className={cn("h-1.5 w-1.5 rounded-full", e.dot)}
                        title={e.title}
                      />
                    ))}
                  </div>
      ...
      <Sheet open={!!selected} onOpenChange={(o) => !o && setSelected(null)}>
        <SheetContent
          side="right"
          className="w-full overflow-y-auto bg-background/95 backdrop-blur-xl sm:max-w-md"
        >
    ...
    <div className="flex h-full flex-col gap-5">
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs">
            {day.redDay && (
              <span className="rounded-full bg-[oklch(0.7_0.12_28/0.15)] px-2 py-0.5 text-[oklch(0.75_0.14_28)]">
                {day.redDay.name}
              </span>
            )}
            {day.tradition && (
            ...
            {day.namedays.length > 0 && (
              <span className="text-muted-foreground">Namnsdag: {day.namedays.join(", ")}</span>
            )}
          </div>
        </div>
        <button
          onClick={onClose}
          className="grid h-9 w-9 place-items-center rounded-full hover:bg-white/[0.05]"
        >
          <X className="h-4 w-4" />
        </button>
      </div>
          ...
          <div key={kind}>
            ...
            <ul className="space-y-1.5">
              {list.map((ev) => (
                <li
                  key={ev.id}
                  className="group flex items-center justify-between rounded-xl border border-border bg-white/[0.02] px-3 py-2"
                >
                  <div className="min-w-0">
                    <div className="truncate text-sm font-medium">{ev.title}</div>
                    {ev.subtitle && (
                      <div className="truncate text-xs text-muted-foreground">{ev.subtitle}</div>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    {ev.amount != null && (
                      <span
                        className={cn(
                          "num text-sm",
                          ev.amount < 0 ? "text-[oklch(0.7_0.14_28)]" : "gold-text",
                        )}
                      >
                        {ev.amount < 0 ? "−" : ""}
                        {sek(Math.abs(ev.amount))}
                      </span>
                    )}
                    {ev.refTable && (
                      <button
      ...
      {(day.totals.hours > 10 || (day.totals.expenses > 1000 && day.totals.gross === 0)) && (
        <div className="rounded-xl border border-[oklch(0.85_0.12_85/0.3)] bg-[oklch(0.85_0.12_85/0.07)] p-3 text-xs">
          <div className="mb-1 flex items-center gap-1.5 text-[oklch(0.85_0.12_85)]">
            <Sparkles className="h-3.5 w-3.5" /> Insikt
          </div>
          {day.totals.hours > 10 && <div>Lång arbetsdag — kom ihåg återhämtning.</div>}
          {day.totals.expenses > 1000 && day.totals.gross === 0 && (
            <div>Hög utgift på en dag utan inkomst.</div>
          )}
        </div>
      )}
    </div>
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle className="display text-2xl">
            {
              {
                shift: "Nytt arbetspass",
                expense: "Ny utgift",
                income: "Ny inkomst",
                reminder: "Ny påminnelse",
...
function inp(extra = "") {
  return cn(
    "w-full rounded-lg border border-border bg-input/40 px-3 py-2 text-sm outline-none focus:border-[oklch(0.85_0.12_85/0.5)]",
    extra,
  );
}
function lbl(s: string) {
  return <div className="mb-1 text-[10px] uppercase tracking-wider text-muted-foreground">{s}</div>;
}
...
function ShiftForm({ date, onSaved }: { date: string; onSaved: () => void }) {
  ...
  useEffect(() => {
    ...
    if (def) {
      setProfileId(def.id);
      setRate(Number(def.hourly_rate ?? 0));
    } else setRate(0);
  }, [profiles.data]);
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const obRules = (def?.ob_rules as OBRule[] | null) ?? DEFAULT_OB_RULES;
      ...
      const { error } = await supabase.from("shifts").insert({
        ...
        work_profile_id: profileId || null,
      });
      if (error) throw error;
    },
    onSuccess: () => {
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const p = profiles.data.find((x: any) => x.id === e.target.value);
              if (p) setRate(Number(p.hourly_rate ?? 0));
            }}
            className={inp()}
          >
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("timeline_events").insert({
        ...
        subtitle: note || null,
        amount,
        occurs_at: new Date(`${date}T12:00:00`).toISOString(),
      });
      if (error) throw error;
    },
    onSuccess: () => {
