  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("shifts").insert({
        user_id: user.id, title, starts_at: startsAt.toISOString(), ends_at: endsAt.toISOString(),
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from("expenses").insert({
        user_id: user.id, amount, category: cat as any,
        description: desc || null, merchant: merchant || null,
