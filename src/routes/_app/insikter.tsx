...
type Insight = { icon: React.ReactNode; severity: "info" | "warning" | "good"; title: string; body: string };
...
function InsikterPage() {
  ...
  const shifts = useQuery({ queryKey: ["ins-shifts"], queryFn: async () => (await supabase.from("shifts").select("*").gte("starts_at", monthStart())).data ?? [] });
  const expenses = useQuery({ queryKey: ["ins-exp"], queryFn: async () => (await supabase.from("expenses").select("*").gte("occurred_at", monthStart())).data ?? [] });
  ...
  const insights = useMemo<Insight[]>(() => {
    ...
    if (income > 0 && spend > income) {
      out.push({
        icon: <AlertTriangle className="h-4 w-4" />, severity: "warning",
    ...
    if (subsTotal > 500) {
      ...
      out.push({
        icon: <TrendingUp className="h-4 w-4" />, severity: "info",
    ...
    if (rate === 0) {
      out.push({
        icon: <Lightbulb className="h-4 w-4" />, severity: "info",
    ...
    if (income > 0 && spend > 0 && spend < income * 0.7) {
      out.push({
        icon: <Sparkles className="h-4 w-4" />, severity: "good",
    ...
    return out;
  }, [shifts.data, expenses.data, profile.data]);

  return (
    <div className="space-y-6">
      <header>
        <div className="text-[11px] uppercase tracking-widest text-muted-foreground">Insikter</div>
        <h1 className="display text-4xl">Din <span className="gold-text">tysta rådgivare</span></h1>
        <p className="mt-1 text-sm text-muted-foreground">Lokala signaler från ditt mönster. Inget skickas någonstans.</p>
      </header>

      <ul className="grid gap-4 md:grid-cols-2">
        {insights.map((ins, i) => {
          const tone =
            ins.severity === "warning" ? "border-[oklch(0.7_0.12_28/0.35)] bg-[oklch(0.7_0.12_28/0.05)] text-[oklch(0.78_0.1_28)]" :
            ins.severity === "good" ? "border-[oklch(0.55_0.1_165/0.35)] bg-[oklch(0.55_0.1_165/0.05)] text-[oklch(0.75_0.1_165)]" :
          ...
          return (
            <li key={i} className="glass rounded-3xl p-6">
              <div className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[11px] ${tone}`}>{ins.icon}{ins.severity === "warning" ? "Varning" : ins.severity === "good" ? "Bra" : "Insikt"}</div>
