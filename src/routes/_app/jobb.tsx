...
function JobbPage() {
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Snabbmotorn lägger in pass på sex olika sätt — vi räknar OB, rast och netto åt dig.
          </p>
        </div>
        <button
          onClick={() => setEngineOpen((v) => !v)}
          className="inline-flex items-center gap-2 rounded-full bg-gradient-to-b from-[oklch(0.88_0.1_85)] to-[oklch(0.7_0.12_75)] px-5 py-2.5 text-sm font-semibold text-background shadow-[0_10px_30px_-10px_oklch(0.78_0.105_85/0.5)]"
        >
      ...
      {engineOpen && (
        <ShiftEngine
          defaultRate={Number(profile.data?.hourlyRate ?? 0)}
          taxRate={Number(profile.data?.taxRate ?? 30)}
          obRules={profile.data?.obRules?.length ? profile.data.obRules : DEFAULT_OB_RULES}
              ...
              <li key={s.id} className="flex items-center gap-4 py-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate font-medium">
                    {s.title || "Arbetspass"}
                    {s.is_extra && (
                      <span className="ml-2 rounded-full bg-[oklch(0.78_0.105_85/0.15)] px-2 py-0.5 text-[10px] text-[oklch(0.85_0.12_85)]">
                        Extra
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-muted-foreground">
                    {sweDate(s.starts_at)} · {sweTime(s.starts_at)}–{sweTime(s.ends_at)} · rast{" "}
                    {s.break_minutes}min
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-[10px] text-muted-foreground">
                    grund {sek(Number(s.base_amount || 0))} + OB {sek(Number(s.ob_amount || 0))}
                  </div>
                </div>
                <button
                  onClick={() => del.mutate(s.id)}
                  className="ml-2 grid h-9 w-9 place-items-center rounded-lg text-muted-foreground hover:bg-white/[0.05] hover:text-[oklch(0.7_0.12_28)]"
                >
...
function ShiftEngine({
  ...
  function applyTemplate(t: ShiftTemplate) {
    setFrom(t.starts_time);
    setTo(t.ends_time);
    setBreakMin(t.break_minutes);
  ...
  const calc = useMemo(() => {
    ...
    for (const p of planned) {
      try {
        const f = p.from ?? from;
        const t = p.to ?? to;
        const br = p.breakMinutes ?? breakMin;
  ...
  const warnings = useMemo<ShiftWarning[]>(
    () =>
      detectWarnings(
        planned.map((p) => ({
          date: p.date,
  ...
  async function saveAll() {
    if (planned.length === 0) {
      toast.error("Inga pass att spara");
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="glass rounded-3xl p-6">
      <div className="mb-4 flex items-center gap-2">
        ...
        <span className="ml-2 rounded-full bg-white/[0.05] px-2 py-0.5 text-[10px] uppercase tracking-wider text-muted-foreground">
          {planned.length} planerade
        </span>
      </div>
      ...
      <div className="mb-4">
        <div className="mb-1.5 flex items-center justify-between">
          <span className="text-[11px] uppercase tracking-wider text-muted-foreground">
            Mallar — klicka för att applicera
          </span>
          ...
          {templates.map((t) => (
            ...
            >
              <span
                ...
                style={{ background: t.color ?? "oklch(0.78 0.105 85)" }}
              />
              <span className="font-medium">{t.name}</span>
              <span className="text-[10px] text-muted-foreground">
                {t.starts_time}–{t.ends_time}
              </span>
              {!t.builtin && (
                <button
                  type="button"
      ...
      <div className="mb-5 rounded-2xl border border-border bg-white/[0.02] p-4">
        ...
        {mode === "kopiera" && (
          <ModeKopiera
            existing={existing}
            onAdd={(items) =>
              setPlanned((prev) => [...prev, ...items].sort((a, b) => a.date.localeCompare(b.date)))
            }
          />
        )}
        {mode === "klistra" && (
            ...
            {planned.map((p, i) => {
              ...
              return (
                ...
                >
                  ...
                  {day?.isRed && (
                    <span className="rounded-full bg-[oklch(0.7_0.12_28/0.15)] px-1.5 py-0.5 text-[10px] text-[oklch(0.78_0.12_28)]">
                      {day.redName ?? "Helg"}
                    </span>
                  )}
                  {day?.isWeekend && !day.isRed && (
                    <span className="rounded-full bg-white/[0.05] px-1.5 py-0.5 text-[10px]">
                      Helg
                    </span>
                  )}
                  ...
                  <span className="num w-20 text-right text-[oklch(0.85_0.12_85)]">
                    {sek(item?.total ?? 0)}
                  </span>
                  <button
                    type="button"
                    onClick={() => removeAt(i)}
                    className="grid h-5 w-5 place-items-center rounded text-muted-foreground hover:bg-white/[0.05]"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </li>
      ...
      {planned.length > 0 && (
        <div className="mb-4 rounded-2xl border border-[oklch(0.78_0.105_85/0.25)] bg-[oklch(0.78_0.105_85/0.05)] p-5">
          <div className="text-[11px] uppercase tracking-widest text-muted-foreground">
            Sammanfattning innan du sparar
          </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
            {redCount > 0 && (
              <Tag color="red">
                {redCount} röd dag{redCount > 1 ? "ar" : ""}
              </Tag>
            )}
          ...
          {warnings.length > 0 && (
            <ul className="mt-3 space-y-1">
              {warnings.map((w, i) => (
                <li
                  key={i}
                  className="flex items-center gap-2 rounded-lg bg-[oklch(0.7_0.12_28/0.08)] px-2.5 py-1.5 text-xs text-[oklch(0.85_0.1_28)]"
                >
  ...
  return (
    <div className="flex items-end gap-2">
      ...
      <span className="text-xs text-muted-foreground">
        Tryck flera gånger för att lägga in dag efter dag.
      </span>
    </div>
  );
}
...
function ModeIntervall({ onAdd }: { onAdd: (dates: string[]) => void }) {
  ...
  function apply() {
    if (e < s) {
      toast.error("Slutdatum före startdatum");
      return;
    }
    const out: string[] = [];
    for (let i = 0; ; i++) {
  ...
  return (
    <div className="grid items-end gap-3 sm:grid-cols-4">
      <Field label="Från">
        <input type="date" value={s} onChange={(ev) => setS(ev.target.value)} className="inp" />
      </Field>
      <Field label="Till">
        <input type="date" value={e} onChange={(ev) => setE(ev.target.value)} className="inp" />
      </Field>
      <label className="flex items-center gap-2 text-sm">
        <input
          type="checkbox"
          checked={skipWeekends}
          onChange={(ev) => setSkipWeekends(ev.target.checked)}
          className="h-4 w-4 accent-[oklch(0.78_0.105_85)]"
        />
        Hoppa över helger
      </label>
...
function ModeMonster({ onAdd }: { onAdd: (dates: string[]) => void }) {
  ...
  function apply() {
    if (days.length === 0) {
      toast.error("Välj minst en veckodag");
      return;
    }
    const out: string[] = [];
        ...
        {WEEKDAYS.map((w) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() =>
                setDays((prev) => (on ? prev.filter((x) => x !== w.i) : [...prev, w.i]))
              }
...
function ModeKopiera({ existing, onAdd }: { existing: any[]; onAdd: (items: Planned[]) => void }) {
  ...
  function apply() {
    if (picked.size === 0) {
      toast.error("Välj minst ett pass");
      return;
    }
    const items: Planned[] = [];
    for (const id of picked) {
      ...
      const newDateBase = new Date(start);
      newDateBase.setDate(newDateBase.getDate() + offsetDays);
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-end gap-3">
        <Field label="Förskjut antal dagar">
          <input
            type="number"
            value={offsetDays}
        ...
        <button
          type="button"
          onClick={apply}
          className="ml-auto rounded-lg bg-[oklch(0.78_0.105_85)] px-4 py-2 text-sm font-semibold text-background"
        >
          Kopiera valda
        </button>
      </div>
        ...
        <ul className="max-h-64 space-y-1 overflow-auto rounded-lg border border-border p-2">
          {sorted.map((s: any) => {
            ...
            return (
              <li key={s.id}>
                <label
                  className={`flex cursor-pointer items-center gap-2 rounded px-2 py-1 text-xs ${on ? "bg-[oklch(0.78_0.105_85/0.1)]" : "hover:bg-white/[0.03]"}`}
                >
                  <input
                    type="checkbox"
                    checked={on}
                  ...
                  <span className="text-muted-foreground">
                    {sweTime(s.starts_at)}–{sweTime(s.ends_at)}
                  </span>
                  <span className="ml-auto num text-[oklch(0.85_0.12_85)]">
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Klistra in ett schema — en rad per dag. T.ex.{" "}
        <code className="rounded bg-white/[0.05] px-1">2025-07-15 14:00-22:00</code> eller{" "}
        <code className="rounded bg-white/[0.05] px-1">Mån 15/7 14-22</code>.
      </p>
      <textarea
        value={text}
      ...
      {preview.length > 0 && (
        <ul className="space-y-0.5 text-xs text-muted-foreground">
          {preview.map((p, i) => (
            <li key={i}>
              · {sweDate(p.date)} {p.from && p.to ? `${p.from}–${p.to}` : "(använder gemensam tid)"}
            </li>
          ))}
        </ul>
      )}
...
function ModeKommando({
  onAdd,
}: {
  onAdd: (dates: string[], times?: { from?: string; to?: string }) => void;
}) {
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">
        Skriv naturligt:
        <code className="ml-1 rounded bg-white/[0.05] px-1">
          07-16 varje måndag, tisdag och fredag i juli
        </code>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="t.ex. 07-16 mån tis fre i juli"
        />
        <button
          type="button"
          onClick={run}
        ...
        {preview && preview.dates.length > 0 && (
          <button
            type="button"
            onClick={() => {
              onAdd(preview.dates, { from: preview.from, to: preview.to });
              setPreview(null);
              setCmd("");
            }}
            className="rounded-lg bg-[oklch(0.78_0.105_85)] px-3 py-2 text-xs font-semibold text-background"
      ...
      {preview && (
        <div className="text-xs text-muted-foreground">
          ...
          {preview.notes.map((n, i) => (
            <p key={i} className="text-[oklch(0.78_0.1_60)]">
              ⚠ {n}
            </p>
          ))}
        </div>
  ...
  return (
    <>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-1 text-xs text-[oklch(0.85_0.12_85)] hover:underline"
      >
      ...
      {open && (
        <div className="absolute right-6 mt-6 flex items-center gap-1 rounded-lg border border-border bg-background p-2 shadow-lg">
          ...
          <button
            type="button"
            onClick={save}
            className="grid h-7 w-7 place-items-center rounded bg-[oklch(0.78_0.105_85)] text-background"
          >
          ...
          <button
            ...
            className="grid h-7 w-7 place-items-center rounded text-muted-foreground"
          >
...
function SummaryStat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div>
      ...
      <div className={`num mt-0.5 text-lg ${accent ? "text-[oklch(0.85_0.12_85)]" : ""}`}>
        {value}
      </div>
    </div>
  );
...
function Tag({
  ...
  const c =
    color === "red"
      ? "bg-[oklch(0.7_0.12_28/0.15)] text-[oklch(0.85_0.1_28)]"
      : color === "amber"
        ? "bg-[oklch(0.78_0.105_85/0.15)] text-[oklch(0.92_0.08_85)]"
        : color === "green"
          ? "bg-[oklch(0.65_0.15_145/0.15)] text-[oklch(0.85_0.13_145)]"
