  ...
  const apply = useMutation({
    mutationFn: async () => {
      ...
      const inserts = expanded.map((e) => {
        ...
        if (endsAt <= startsAt) endsAt = new Date(endsAt.getTime() + 86400000);
        const c = calculateShift({ startsAt, endsAt, breakMinutes: e.breakMinutes, hourlyRate: rate, obRules: DEFAULT_OB_RULES });
        return {
          user_id: user.id, title: preset.name, starts_at: startsAt.toISOString(), ends_at: endsAt.toISOString(),
          break_minutes: e.breakMinutes, hourly_rate: rate,
